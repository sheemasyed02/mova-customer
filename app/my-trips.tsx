...
export default function MyTripsScreen() {
  ...
  const getFilteredTrips = () => {
    ...
    switch (activeTab) {
      case 'upcoming':
        filtered = filtered.filter(trip => ['confirmed', 'pending_approval', 'payment_pending'].includes(trip.status));
        break;
      case 'ongoing':
        filtered = filtered.filter(trip => trip.status === 'ongoing');
        break;
      case 'completed':
      ...
      <View style={styles.tripContent}>
        ...
        <View style={styles.tripInfo}>
          ...
          {activeTab === 'ongoing' && item.liveStatus && (
            <View style={styles.liveStatusContainer}>
              ...
              <Text style={styles.liveStatus}>
                {item.liveStatus === 'picked_up' ? 'Picked up' : 'Ongoing'}
              </Text>
              {item.endTimeHours && (
                <Text style={styles.endTime}>• End trip in {item.endTimeHours}h</Text>
              )}
            </View>
          )}
          ...
          <View style={styles.tripDates}>
            ...
            <View style={styles.dateRow}>
              <Ionicons name="location" size={14} color={Colors.accent.blue} />
      ...
      case 'ongoing':
        buttons.push(
          ...
          <TouchableOpacity key="support" style={[styles.actionButton, styles.emergencyButton]}>
            <Text style={[styles.actionButtonText, styles.emergencyButtonText]}>Trip Support</Text>
          </TouchableOpacity>
        );
        break;
